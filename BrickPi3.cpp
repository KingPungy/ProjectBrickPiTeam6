...
int BrickPi3::spi_read_32(uint8_t msg_type, uint32_t &value) {
    ...
    value = ((spi_array_in[4] << 24) | (spi_array_in[5] << 16) |
             (spi_array_in[6] << 8) | spi_array_in[7]);
    return ERROR_NONE;
}
...
int BrickPi3::spi_read_string(uint8_t msg_type, char *str, uint8_t chars) {
    ...
    // assign error to the value returned by spi_transfer_array, and if not 0:
    if (int error =
            spi_transfer_array(chars + 4, spi_array_out, spi_array_in)) {
        return error;
    }
    if (spi_array_in[3] != 0xA5) {
        return ERROR_SPI_RESPONSE;
...
int BrickPi3::detect(bool critical) {
    ...
    if (error = get_manufacturer(str)) {
        if (critical) {
            fatal_error(
                "detect error: get_manufacturer failed. Perhaps the BrickPi3 "
                "is not connected, or the address is incorrect.");
        } else {
            return error;
        }
    }
    if (strstr(str, "Dexter Industries") != str) {
        if (critical) {
            fatal_error(
                "detect error: get_manufacturer string is not 'Dexter "
                "Industries'");
        } else {
    ...
    if (strstr(str, FIRMWARE_VERSION_REQUIRED) != str) {
        if (critical) {
            sprintf(ErrorStr,
                    "detect error: BrickPi3 firmware needs to be version %sx "
                    "but is currently version %s",
...
int BrickPi3::get_version_hardware(char *str) {
    ...
    sprintf(str, "%d.%d.%d", (value / 1000000), ((value / 1000) % 1000),
            (value % 1000));
}
...
int BrickPi3::set_sensor_type(uint8_t port, uint8_t type, uint16_t flags,
...
int BrickPi3::transact_i2c(uint8_t port, i2c_struct_t *i2c_struct) {
    ...
    switch (port) {
        ...
        default:
            fatal_error(
                "transact_i2c error. Must be one sensor port at a time. "
                "PORT_1, PORT_2, PORT_3, or PORT_4.");
    }
    ...
    return spi_transfer_array((5 + i2c_struct->length_write), spi_array_out,
                              spi_array_in);
}
...
int BrickPi3::get_sensor(uint8_t port, void *value_ptr) {
    ...
    switch (port) {
        ...
        default:
            fatal_error(
                "get_sensor error. Must be one sensor port at a time. PORT_1, "
                "PORT_2, PORT_3, or PORT_4.");
    }
    ...
    // assign error to the value returned by spi_transfer_array, and if not 0:
    if (int error = spi_transfer_array(spi_transfer_length, spi_array_out,
                                       spi_array_in)) {
        return error;
    }
    ...
    if (!(spi_array_in[4] == SensorType[port_index] ||
          (SensorType[port_index] == SENSOR_TYPE_TOUCH &&
           (spi_array_in[4] == SENSOR_TYPE_TOUCH_NXT ||
            spi_array_in[4] == SENSOR_TYPE_TOUCH_EV3)))) {
        return ERROR_SENSOR_TYPE_MISMATCH;
    }
    // If the sensor value(s) is not valid (still configuring the sensor, or
    // error communicating with the sensor)
    ...
    // For each sensor type, copy the value(s) into the corresponding structure
    // value(s)
    if (SensorType[port_index] == SENSOR_TYPE_TOUCH ||
        ...
        Value->adc6 =
            (((spi_array_in[8] >> 4) & 0x0F) | (spi_array_in[7] << 4));
        ...
        for (uint8_t v = 0; v < 4; v++) {
            switch (spi_array_in[6 + v]) {
                ...
                case 8:
                    Value->remote[v] =
                        REMOTE_BIT_RED_DOWN | REMOTE_BIT_BLUE_DOWN;
                    break;
                case 9:
                ...
                case 11:
                    Value->remote[v] =
                        REMOTE_BIT_BLUE_UP | REMOTE_BIT_BLUE_DOWN;
                    break;
                default:
                    Value->remote[v] = 0;
                    break;
        ...
        Value->reflected_red =
            ((spi_array_in[7] << 2) | ((spi_array_in[11] >> 6) & 0x03));
        Value->reflected_green =
            ((spi_array_in[8] << 2) | ((spi_array_in[11] >> 4) & 0x03));
        Value->reflected_blue =
            ((spi_array_in[9] << 2) | ((spi_array_in[11] >> 2) & 0x03));
        Value->ambient = ((spi_array_in[10] << 2) | (spi_array_in[11] & 0x03));
    } else if (SensorType[port_index] ==
        ...
        // Value-> = ((spi_array_in[12] << 8) | spi_array_in[13]); not sure what
        // this value is
...
int BrickPi3::set_motor_position_relative(uint8_t port, int32_t position) {
    for (uint8_t p = 1; p <= PORT_D; p <<= 1) {
        if (port & p) {
            int32_t encoder = 0;
            // assign error to the error value returned by get_motor_encoder,
            // and if not 0:
...
int BrickPi3::get_motor_status(uint8_t port, uint8_t &state, int8_t &power,
    ...
    switch (port) {
        ...
        default:
            fatal_error(
                "get_motor_status error. Must be one motor port at a time. "
    ...
    position = ((spi_array_in[6] << 24) | (spi_array_in[7] << 16) |
                (spi_array_in[8] << 8) | spi_array_in[9]);
...
int BrickPi3::reset_motor_encoder(uint8_t port) {
    ...
    if (error) {
        fatal_error(
            "reset_motor_encoder error. Must be one or more motor ports. "
            "PORT_A, PORT_B, PORT_C, and/or PORT_D.");
    }
    return ERROR_NONE;
}
...
int BrickPi3::reset_motor_encoder(uint8_t port, int32_t &value) {
    value = 0;
    // assign error to the error value returned by get_motor_encoder, and if not
    // 0:
...
int BrickPi3::get_motor_encoder(uint8_t port, int32_t &value) {
    ...
    switch (port) {
        ...
        default:
            fatal_error(
                "get_motor_encoder error. Must be one motor port at a time. "
                "PORT_A, PORT_B, PORT_C, or PORT_D.");
    }
    uint32_t Value;
